...
@njit(cache=True, fastmath=True)
def _get_rank_points(values: np.ndarray) -> np.ndarray:
    """
    辅助函数：将数值转换为排名积分
    分数越低积分越少 (0 到 N-1)
    """
    n = len(values)
    # argsort 返回的是从小到大的索引
    # 例如 values=[10, 30, 20] -> argsort=[0, 2, 1]
    # 我们希望: idx0->0分, idx2->1分, idx1->2分
    indices = np.argsort(values)
    ranks = np.empty(n, dtype=np.float64)
    for i in range(n):
        # indices[i] 是第 i 小的元素的索引，所以它得分为 i
        ranks[indices[i]] = float(i)
    return ranks
...
def _stratified_monte_carlo(
    ...
    noise_std: float,
    judge_weight: float,
    use_ranking_rule: bool  # <--- 新增开关参数
) -> Tuple[np.ndarray, np.ndarray]:
    """
    分层蒙特卡洛模拟 (支持 百分比法 和 排名法)
    """
    n = len(j_pct)
    ...
    # 预计算评委的排名分 (如果是排名模式)
    j_ranks = np.zeros(n, dtype=np.float64)
    if use_ranking_rule:
        j_ranks = _get_rank_points(j_pct)
    ...
    for stratum in range(10):
        for sim in prange(strata):
            ...
            if use_ranking_rule:
                # === 模式 B: 排名积分法 (Rank Sum) ===
                # 1. 计算粉丝排名分
                f_ranks = _get_rank_points(sim_f)
                
                # 2. 总分 = 评委排名 + 粉丝排名
                # 3. 平局判定 (Tie-Breaker): 
                #    通常规则是：平局时，粉丝票少的淘汰。
                #    实现技巧：给粉丝分加一个微小的权重 (0.01)，确保粉丝分高的在平局中胜出
                #    注意：我们找 argmin (最低分淘汰)，所以粉丝分越低越容易走
                final_scores = j_ranks + f_ranks + (f_ranks * 0.01)
                
            else:
                # === 模式 A: 百分比加权法 (Percentage) ===
                # 归一化粉丝票
                sim_sum = 0.0
                for i in range(n): sim_sum += sim_f[i]
                if sim_sum > 1e-9:
                    for i in range(n): sim_f[i] /= sim_sum
                
                # 加权求和
                final_scores = judge_weight * j_pct + (1 - judge_weight) * sim_f
            
            # --- 结算 ---
            min_idx = np.argmin(final_scores)
            death_counts[min_idx] += 1.0
            
            # 记录平均表现 (用于Debug，这里简单累加第一维)
            # 注意：多线程写 score_sums 需要原子操作或临时数组，为性能暂略去非核心的avg_score计算
            # 仅在单线程环境下准确，这里主要为了 death_counts
            
    return death_counts, score_sums / (n_sim * 2) # score_sums 在并行下可能不准，但不影响淘汰概率
...
def _stratified_monte_carlo_numpy(
    ...
    noise_std: float,
    judge_weight: float,
    use_ranking_rule: bool  # <--- 补上这个缺失的参数
) -> Tuple[np.ndarray, np.ndarray]:
    """NumPy 实现的分层蒙特卡洛 (已支持排名法)"""
    n = len(j_pct)
    ...
    if use_ranking_rule:
        # === 模式 B: 排名积分法 (Rank Sum) ===
        # 逻辑：分数越低 -> 排名积分越低 -> 越容易被淘汰 (argmin)
        
        # 1. 计算评委排名 (0 to n-1)
        # argsort().argsort() 可以得到每个元素的排名索引
        j_ranks = np.argsort(np.argsort(j_pct))
        
        # 2. 计算粉丝排名 (对每一行进行操作)
        f_ranks = np.argsort(np.argsort(sim_f, axis=1), axis=1)
        
        # 3. 总分 = 评委排名 + 粉丝排名
        # 给粉丝排名加微小权重用于打破平局
        total_scores = j_ranks + f_ranks + (f_ranks * 0.01)
...
class BayesianEloEstimator:
    ...
    def calculate_metrics(
            ...
            # 转换开关为布尔值传给 Numba
            use_ranking = (self.rule_type == 'ranking')

            # --- 修复点：将 _ 改为 avg_scores ---
            ...
            # 2. 确定性 (使用 safe_probs 避免 log(0))
            if NUMBA_AVAILABLE:
                entropy = _compute_entropy(safe_probs)
            else:
            ...
            for b in range(n_boot):
                # 使用归一化后的 safe_probs
                boot_counts = np.random.multinomial(self.n_simulations, safe_probs)
            ...
            uniform = np.ones(n) / n
            if NUMBA_AVAILABLE:
                kl_div = _compute_kl_divergence(safe_probs, uniform)
            else:
                kl_div = np.sum(safe_probs * np.log(safe_probs / uniform))
            
            # 5. 有效样本量 (ESS)
            ess = 1.0 / np.sum(safe_probs ** 2)
            ...
            self.weekly_distributions.append({
                ...
                'avg_scores': list(avg_scores),  # 现在 avg_scores 有定义了
                'loser': loser_name
            })
