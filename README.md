...
- 采用带权重的双对数形式：
  $$S_{total} = w_j\cdot P_{judge} + w_f\cdot \frac{\ln(1 + \lambda \cdot P_{fan})}{\ln(1 + \lambda)} + \mathbb{I}_{\text{bonus}} \cdot \beta$$

其中 $w_j=0.8$, $w_f=0.2$，$\lambda=100$，$\beta=0.05$
理由：
...
- **进步奖励**：每周计算每个选手的评委评分与上周的差值，进步最大的选手获得0.05加分；

### 实现与验证：

我们在 `new_judgements.py` 中实现了上述评分系统：
- 我们将选手的 elo 评分作为高斯分布的均值，标准差与 RD 不确定度相关，模拟了 2000 场突然死亡对决，统计每位选手的胜率。

我们还对历史数据进行了模拟。其中：
1. **138 名 (46.78%)** 本来被淘汰的选手，在新规则下排名高于倒数第二，即无需参加突然死亡模式。
2. **42 名（14.24%）** 现实中被淘汰，新规则下进入突然死亡模式的选手打赢突然死亡而复活。

## 总结：我们做了什么
[数据处理部分]：
通过正则表达式解析 results 字段中的文本信息，我们精确了界定每位选手的参赛生命周期；针对 "Withdrew"（主动退赛）的特殊情况，脚本不依赖静态标签，而是遍历分数列（score_cols），动态寻找该选手拥有非零分数的最后活跃周。这一部分选手在后续预计得票率计算中将参与到其参与的最后一期节目裁判总给分里，但是不影响下一期的得票率计算。
完成结构转换后，我们计算了两个相对指标，以消除不同赛季裁判打分尺度不一致的影响：